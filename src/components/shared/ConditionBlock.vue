<template>
    <div class="d-flex w-100 gap-4 align-items-start flex-wrap">
      ...
      <select v-if="value.field != 'all'" v-model="value.operator" class="border  rounded p-2">
      ...
      <div v-if="value.field != 'all'">
        ...
        <div v-else>
          <div v-if="!is_operand_dynamic_field" class="d-flex">
            <span @click="is_operand_dynamic_field = true" class="d-inline-flex align-items-center justify-content-center px-3 border" title="Switch between input value or dynamic fields">&#9776;</span>
            <input v-model="value.operand" type="text" class="border w-240px p-2">
          </div>
          <div v-if="is_operand_dynamic_field" class="d-flex">
            <span @click="is_operand_dynamic_field = false" class="d-inline-flex align-items-center justify-content-center px-3 border" title="Switch between input value or dynamic fields">&#9776;</span>
            <select v-model="value.operand" class="border w-240px p-2">
              <option v-for="(field, field_i) in project_fields" :key="field_i">{{field}}</option>
            </select>
          </div>
        </div>
